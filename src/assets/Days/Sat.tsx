...
export default function Saturday() {
    ...
    const [saturday, setSaturday] = useState<Task[] | null>(null);
    ...
    useEffect(() => {
      const data = localStorage.getItem("list-saturday");
      if (data) {
        setSaturday(JSON.parse(data));
        setFirstTask(true)
      } else {
        setSaturday([]);
        setFirstTask(false)
    ...
    useEffect(() => {
      if (saturday !== null) {
        localStorage.setItem("list-saturday", JSON.stringify(saturday));
      }
    }, [saturday]);
    ...
    const changeData = () => {
      ...
      if (nameTask && time && category) {
        const taskExists = saturday?.some(
   ...
   const Daydata = (saturday || []).map(element => {
    const  removeTask = (id: string | number) => {
       let filterArray = saturday?.filter(item => item.id !== id)
       setSaturday(filterArray!)
    }
    return (
  ...
  const Allday = selday.map(element => { 
    ...
    return <li className={element.name === "Sat" ? "day-btn-selected": "day-btn"} ><Link  to={element.day} >{element.name}</Link></li>
   });
    return (
        <>
