...
export default function Friday() {
    ...
    const [friday, setFriday] = useState<Task[] | null>(null);
    ...
    useEffect(() => {
      const data = localStorage.getItem("list-friday");
      if (data) {
        setFriday(JSON.parse(data));
        setFirstTask(true)
      } else {
        setFriday([]);
        setFirstTask(false)
    ...
    useEffect(() => {
      if (friday !== null) {
        localStorage.setItem("list-friday", JSON.stringify(friday));
      }
    }, [friday]);
   ...
   const Daydata = (friday || []).map(element => {
    const  removeTask = (id: string | number) => {
       let filterArray = friday?.filter(item => item.id !== id)
       setFriday(filterArray!)
    }
    return (
  ...
  const Allday = selday.map(element => { 
    ...
    return <li className={element.name === "Fri" ? "day-btn-selected": "day-btn"} ><Link  to={element.day} >{element.name}</Link></li>
   });
    return (
        <>
