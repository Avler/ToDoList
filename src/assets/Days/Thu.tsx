...
export default function Thusday() {
    ...
    const [thusday, setThusday] = useState<Task[] | null>(null);
    ...
    useEffect(() => {
      const data = localStorage.getItem("list-thusday");
      if (data) {
        setThusday(JSON.parse(data));
        setFirstTask(true)
      } else {
        setThusday([]);
        setFirstTask(false)
    ...
    useEffect(() => {
      if (thusday !== null) {
        localStorage.setItem("list-thusday", JSON.stringify(thusday));
      }
    }, [thusday]);
   ...
   const Daydata = (thusday || []).map(element => {
    const  removeTask = (id: string | number) => {
       let filterArray = thusday?.filter(item => item.id !== id)
       setThusday(filterArray!)
    }
    return (
  ...
  const Allday = selday.map(element => { 
    ...
    return <li className={element.name === "Thu" ? "day-btn-selected": "day-btn"} ><Link  to={element.day} >{element.name}</Link></li>
   });
    return (
        <>
