...
export default function Wednesday() {
    ...
    const [wednesday, setWednesday] = useState<Task[] | null>(null);
    ...
    useEffect(() => {
      const data = localStorage.getItem("list-wednesday");
      if (data) {
        setWednesday(JSON.parse(data));
        setFirstTask(true)
      } else {
        setWednesday([]);
        setFirstTask(false)
    ...
    useEffect(() => {
      if (wednesday !== null) {
        localStorage.setItem("list-wednesday", JSON.stringify(wednesday));
      }
    }, [wednesday]);
    ...
    const changeData = () => {
      ...
      if (nameTask && time && category) {
        const taskExists = wednesday?.some(
   ...
   const Daydata = (wednesday || []).map(element => {
    const  removeTask = (id: string | number) => {
       let filterArray = wednesday?.filter(item => item.id !== id)
       setWednesday(filterArray!)
    }
    return (
  ...
  const Allday = selday.map(element => { 
    ...
    return <li className={element.name === "Wed" ? "day-btn-selected": "day-btn"} ><Link  to={element.day} >{element.name}</Link></li>
   });
    return (
        <>
